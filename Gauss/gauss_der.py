...
def gauss_der(img, sigma):
    ...
    new_color_tableY = []
    t = time.time()
    color_tableX = np.array([[QColor(imageX.pixel(i, j)).getRgb()[:3] for j in range(imageX.height())] for i in range(imageX.width())])
    color_tableY = np.array([[QColor(imageX.pixel(i, j)).getRgb()[:3] for j in range(imageX.height())] for i in range(imageX.width())])
        ...
        for j in range(img.height()):
            ...
            for c in range(3):
                if(i + n < img.width() and j + n < img.height() and i - n + 1 >= 0 and j - n + 1 >= 0):
                    ...
                    result_colorX[c] = (matrixX * color_tableY[i - n + 1: i + n, j - n + 1: j + n, c]).sum() / matrixX.sum()
            ...
            print(i, j, ": ", color_tableX[i][j][0], " -> ", int(result_colorX[0]), "\t\t\t", color_tableY[i][j][0], " -> ", int(result_colorY[0]) )
    ...
    for i in range(img.width()):
        for j in range(img.height()):
            imageX.setPixel(i, j, qRgb(int(new_color_tableX[i][j][0]), int(new_color_tableX[i][j][1]), int(new_color_tableX[i][j][2])))
            imageY.setPixel(i, j, qRgb(int(new_color_tableY[i][j][0]), int(new_color_tableY[i][j][1]),int(new_color_tableY[i][j][2])))




    # pixs = [[64, 77, 85, 87, 64], [75,29,102,181,179], [56, 72, 183, 253, 226], [50, 96, 218, 237, 206], [59, 86, 187, 235, 227]]
